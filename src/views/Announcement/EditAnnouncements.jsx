...
const EditAnnouncements = ({
  ...
  const handleEditing = async (values) => {
    ...
    try {
      const response = await api.patch(
        `/announcement/${selectedAnnouncements?.data?.id}`,
        formData,
        {
          headers: {
      ...
      console.error(error);
      message.error("Failed to create announcements.");
    } finally {
      setLoading(false);
      setShowFiles(false);
      setFilesToUpload([]);
  ...
  // }, []);
  useEffect(() => {
    console.log(selectedAnnouncements, "secllllllll");

    if (selectedAnnouncements) {
      console.log(selectedAnnouncements, "secllllllll");

      form.setFieldsValue({
        title: selectedAnnouncements?.data?.title,
        description: selectedAnnouncements?.data?.description,
        tags: selectedAnnouncements?.data?.tags?.split(","),
      });
    }
  }, [selectedAnnouncements, form]);
      ...
      >
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Please input your announcements title!",
            },
          ]}
        >
        ...
        <Image
          ...
          src={selectedAnnouncements?.data?.image}
        />
        <Form.Item name="image" label="Image" className="mt-3">
          ...
          >
            <Button icon={<UploadOutlined />}>
              Select announcements attachment
            </Button>
          </Upload>
        </Form.Item>
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Please input your announcements content!",
            },
          ]}
        >
          <ReactQuill
