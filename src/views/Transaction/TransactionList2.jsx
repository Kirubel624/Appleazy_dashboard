...
const TransactionsList2 = ({ collapsed }) => {
  ...
  async function searchData2(value = "") {
    try {
      const res2 = await api.get(
        "/transaction/getAllAgentTransactions?search=" + value
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "profileImage",
      render: (text, recored) => {
        return <p>{recored?.User?.username}</p>;
      },
      sorter: true,
    },
    {
      ...
      render: (text, recored) => {
        return (
          <div>
            <Select
              defaultValue={text}
              onChange={(info) => handlePaymentGateway(info, recored.id)}
  ...
  const handlePaymentGateway = async (value, id) => {
    console.log(value, id);
    try {
      setLoading(true);
      const formData = new FormData();
