...
import { NavLink } from 'react-router-dom';
    ...
    const onAdd = async(e)=>{
      e.preventDefault();
        ...
        const data = await assistantsService.assistantsDo({method:'add_list_to_assistant',payload:{data:assistantsData2}})
    ...
    const handleAddToList = (e)=>{
      // e.preventDefault()
      setAssistantsData2([{...form.getFieldsValue()?.assistant,_id:new Date().getTime()},...assistantsData2])
      handleReset()
    }
    ...
    const onClick = ({ key }, record) => {
      if (key == 'edit') {
        ...
        const data = assistantsData2.filter((assistant)=>assistant._id !== record._id)
    ...
    {assistantsData2.length>0 && <CommonTable
      ...
      <ButtonStyle>
          ...
          {!mode&&<button disabled={assistantsData2.length==0} onClick={onAdd} className={assistantsData2.length>0?"":'disable'} type='submit'  >
            Submit
          </button>}
        </ButtonStyle>
      </FormStyle>
